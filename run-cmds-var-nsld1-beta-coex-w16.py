...
def run(cmd, lam1, lam21,lam22, nmld, nsld1, nsld2, beta, tt, tf):
    path = f" --path ./log/log-var-beta-{lam1:.4f}-{lam21:.4f}-{lam22:4f}-{nmld}-{nsld1}-{nsld2}-{beta:.3f}-{tt}-{tf}.csv --snum 2"
    print(cmd + path)
    os.system(cmd + path)
...
with get_context("fork").Pool(8) as pool:
    for nsld1 in range(0, 21, 1):
        ...
        for beta in np.arange(0.00, 1.01, 0.01):
            cmd_run = cmd + f"--nsld1 {nsld1} --nslhtod2 {nsld2} --nmld {nmld} --lam1 {lam1} --lam21 {lam21} --lam22 {lam22} --beta {beta} --mldW1 16 --rt 1000000"
