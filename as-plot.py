...
for w in w_range:
    ...
    for nsld in np.arange(5, 35, 5):
        ...
        lam21 = lam210 / nsld1
        lam22 = lam220 / nsld2
        file = f"./log-as/log-var-W-{w}-{lam1:.4f}-{lam21:.4f}-{lam22:4f}-{nmld}-{nsld1}-{nsld2}-{beta:.3f}-{tt}-{tf}.csv"
...
plt.xlabel("nsld")
plt.savefig("all_sat_acd_vs_nsld.png")
...
plt.xlabel("nsld")
plt.savefig("all_sat_qd_vs_nsld.png")