...
def run(cmd, lam1, lam21,lam22, nmld, nsld1, nsld2, beta, tt, tf, mldW):
    log = f" --path ./log-as/log-var-W-{mldW}-{lam1:.4f}-{lam21:.4f}-{lam22:4f}-{nmld}-{nsld1}-{nsld2}-{beta:.3f}-{tt}-{tf}.csv --snum 5 --rt 20000000 --st 10000000"
    print(cmd + log)
    os.system(cmd+log)
...
with get_context("fork").Pool(16) as pool:
    for mldW in [16, 32, 64, 128, 256, 1024]:
